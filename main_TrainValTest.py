...
def train(dataset_conf, train_conf, results_path):
    ...
    for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        X_train, _, y_train_onehot, X_test, _, _ = get_data(
        ...
        X_train,y_train_onehot = cut_data.cut(dataset_conf=dataset_conf,sub=sub,LOSO=LOSO,dataset=dataset)
        ...
        for train in range(n_train): # How many repetitions of training for subject i.
            ...
            few_shot = 11
            model_output = model.predict(X_test[:few_shot])
            y_updated = y_train_onehot
            x_updated = X_train
            print(y_updated.shape)
            print(x_updated.shape)
            for pos in range(0,few_shot):
                output = model_output[pos]
                if max(output) > 0.9: #default is 0.9
                    # print(x_updated)
                    to_append = np.expand_dims(X_test[pos],axis=0)
                    x_updated = np.append(x_updated,to_append,axis=0)
                    print(output.argmax(axis=-1))
                    if output.argmax(axis=-1) == 0:
                        y_updated = np.append(y_updated,[[1.,0.,0.,0.]],axis=0)
                    if output.argmax(axis=-1) == 1:
                        y_updated = np.append(y_updated,[[0.,1.,0.,0.]],axis=0)
                    if output.argmax(axis=-1) == 2:
                        y_updated = np.append(y_updated,[[0.,0.,1.,0.]],axis=0)
                    if output.argmax(axis=-1) == 3:
                        y_updated = np.append(y_updated,[[0.,0.,0.,1.]],axis=0)

            print(y_updated.shape)
            print(x_updated.shape)

            history = model.fit(x_updated, y_updated, validation_data=(X_train, y_train_onehot), 
                                epochs=epochs, batch_size=batch_size, callbacks=callbacks, verbose=2)
...
def test(model, dataset_conf, results_path, allRuns = True):
    ...
    for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        for seed in range(len(runs)): 
            ...
            # Predict MI task
            y_pred = model.predict(X_test[11:]).argmax(axis=-1)
            inference_time = (time.time() - inference_time)/X_test[11:].shape[0]
            # Calculate accuracy and K-score          
            labels = y_test_onehot.argmax(axis=-1)[11:]
...
def run():
    ...
    default_path = "D:/atcnet/bci2a_mat/"
    parser.add_argument('--path', type=str, default=default_path, help="Base path for datasets (default: 'D:\\ZSL\\BCI_Data\\Handy_Rehab\\')")
